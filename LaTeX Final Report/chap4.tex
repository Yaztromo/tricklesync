...
In specific, given a value $n$ representing the number of time intervals to look ahead into, the probability function $P(x)$ that generates the probability that the user will access a record on the mobile device during the fixed-size interval of the day $x$, $\Delta t$ is the time since the last synchronization session, and $C_{byte_{net}}$ is the cost of transferring one byte of data on the selected network, then we define the function $G$ as:
\begin{equation}
G(x) =  \frac{\Bigg(\displaystyle{\sum_{i=1}^n \frac{2^{n-i} \cdot P(x+i)}{2^n-1}}\Bigg)\cdot\Delta t}{C_{byte_{net}}} \label{threshold_function}
\end{equation}
...
\begin{equation}
\displaystyle{\lim_{C_{byte_{net}} \rightarrow 0} \frac{\Bigg(\displaystyle{\sum_{i=1}^n \frac{2^{n-i} \cdot P(x+i)}{2^n-1}}\Bigg)\cdot \Delta t}{C_{byte_{net}}} = \infty\label{threshold_limit_eqn}}
\end{equation}

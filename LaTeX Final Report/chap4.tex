...
\begin{equation}
G(x) =  \frac{\Bigg(\displaystyle{\sum_{i=1}^n \frac{2^{n-i} * P(x+i)}{2^n-1}}\Bigg)* \Delta t}{C_{byte_{net}}}
...
In order to determine whether cost improvements would occur when the threshold is increased, decreased, or by staying at the current threshold point, the ETS algorithm tests three different threshold points, within a range consisting of an \emph{upper point}, a \emph{lower point}, and \emph{midpoint}.

The threshold evaluation subsystem is composed of two algorithms:  finding suitable threshold initialization values, and updating the threshold.

\subsubsection{Threshold Initialization}

\begin{equation}
T_{init} = \frac{3600}{max(C_{byte_{net}})}
\end{equation}\label{threshold_init}

\begin{equation}
T_{upper} = \frac{43200}{max(C_{byte_{net}})}
\end{equation}
...
\subsubsection{Updating the Threshold}

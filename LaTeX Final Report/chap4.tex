...
In the event that in evaluating $T_{upper}$, $T_{midpoint}$, and $T_{lower}$ their associated costs are equal, it becomes difficult to make a decision as how to react.  When this situation occurs, it may indicate that we're on a plateau, but with no way to evaluate this fact.  This may not be a significant problem when the range between $T_{lower}$ and $T_{upper}$ is very small; indeed, this situation ideally eventually occurs, as it indicates we've found a local minima.  However, if the range is still large (bigger that $\epsilon$), we have no way of knowing whether it's because the plateau is a suitable minima, or if it's in fact closer to a maxima.  Figure \ref{threshold_problem_a} illustrates this problem.

If the range between $T_{lower}$ and $T_{upper}$ is large, no progress will be made, as the midpoints between both $T_{lower}$ and $T_{midpoint}$, and $T_{midpoint}$ and $T_{upper}$ are equally poor candidates.  

% How can we fix this situation?
% Idea:  if the cost of all points is within some small percentage of each other, select the midpoint but 
% increase the upper by 10%, and decrease the lower by 10%.

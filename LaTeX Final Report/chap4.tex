...
In practice, it appears that the ETS algorithm's existing threshold management algorithms are able to sufficiently deal with this situation.  While a potentially good candidate may initially be avoided due to a random event causing a spike in the cost, the algorithm in the long run should tend back down towards this point if it does indeed indicate a cost benefit.  In figure \ref{threshold_problem_c} for example, we'll select the midpoint, reduce the threshold set magnitude by 20\%, and assuming no other random spikes in the future, we'll tend back down towards $L_T$ as we "slide" down the slope of the graph.  As such, mechanisms to better mitigate this problem are left to future research.

\paragraph{Not guaranteed to find the global minima}

It should be stressed that the ETS algorithm is not designed to find the threshold value which exhibits the absolute minimum cost.  In practice, the range of threshold values which permit synchronization can easily surpass $1.0 * 10^{308}$ (the order of magnitude for a 64-bit floating point number, as is used in the current implementation of the algorithm).  Mathematically, as the cost per byte of transfer on the current network approaches $0$, the cost will approach infinity, as expressed in equation \ref{threshold_limit_eqn}.

\begin{equation}
\displaystyle{\lim_{C_{byte_{net}} \rightarrow 0} \frac{\Bigg(\displaystyle{\sum_{i=1}^n \frac{2^{n-i} * P(x+i)}{2^n-1}}\Bigg)* \Delta t}{C_{byte_{net}}} = \infty\label{threshold_limit_eqn}}
\end{equation}

Due to this fact, the ETS algorithm doesn't guarantee that it will find the absolute minimal cost threshold point, but instead attempts to find a suitable local minima through what is effectively a binary search of the overall threshold space.  While this point may not be globally minimal, it is felt that the approach taken in the ETS algorithm both runs within a reasonable amount of time, and produces a result that will be generally better than existing ad-hoc mechanisms, such as relying on user-initiated synchronization, or synchronizing after every fixed unit of time.

Figure \ref{threshold_problem_d} demonstrates an example threshold value graph  with both a local minimum and an absolute minimum.  Because the local minimum traverses significantly more threshold point values than the global minimum point, the probability of the ETS algorithm falling into this region as minimal is higher than for the global minimum, which only exists within a relatively narrow range of threshold points, and is surrounded by high-cost areas on either side.

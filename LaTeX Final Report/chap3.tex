...
\section{Limitations}
 ...
 - Synchronization is only in one direction.
 - No attention is paid to conflict resolution costs (conflict resolution is assumed to be free)
 - Packets are not compressed, and are assumed to be fully transferred.  There is no ability to encode only the record deltas to reduce the transfer size of individual records.  This can improve the cost savings of all the protocols (except null sync).
  - Synchronization always resolves within one tick within the simulator (the sync doesn't run within simulator time)

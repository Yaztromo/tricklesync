...
For events which may occur multiple times in a single tick, a Poisson Process is utilized \cite{RefWorks:38, RefWorks:40}.  During simulation set-up, an expected number of events per hour ($E_{hour}$) is provided, along with a maximum number of permitted events per interval ($N_{max}$).  From these two values, a series of cumulative event probabilities is calculated, in the range of $[0..N_{max}]$.  That is, the probability of exactly $n$ events occurring during an interval with arrival rate $r$ and for interval $t$ is calculated as:
\begin{equation}
P(n) = \sum_{k=0}^n \frac{(rt)^k}{k!}e^{-rt}\label{poisson1}
\end{equation}

Every simulated second, an Poisson trial occurs by selecting a uniformly distributed random number 
$R$ from the PRNG, and finding the smallest $n$ such that:
\begin{equation}
P(n)>R\label{poissontrial}
\end{equation}

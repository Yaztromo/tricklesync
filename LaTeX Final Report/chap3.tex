...
Tick events are used for simulated elements which require evaluation within regular, one-second intervals.  These include all simulated elements which use either a Bernoulli Process (\ref{bernoulli}) or a Poisson Process (\ref{poisson}), along with any elements which use their own distribution or process which must be evaluated at regular intervals which cannot be pre-computed.  During every iteration of the tick loop within the time controller, every registered tick listener has its \texttt{activateTick} method called, passing with it the current tick value.  Note that tick listeners are no guaranteed to be called in any specific order.
...
The simulation controller ties the entire simulation engine together by providing the facility to load the simulation parameters from a file, and the permitting a multi-day simulation to occur.  It uses an instance of the time controller (\ref{time_controller}) to run multiple single-day simulations, up to a user-specified maximum number of days.

As part of its duties, the synchronization controller resets the time controller for each new day, and calculates and maintains the values required to calculate the variance ($\sigma$) of the cost from day-to-day.  The variance is calculated using the following algorithm described by Knuth in \cite{RefWorks:42}, section X, as algorithm X.  This algorithm has been modified slightly in the simulator to work with linear equations instead of real-numbered values, so as to generate results appropriate to the simulators solutions.
...
No attention is paid in the cost calculation to mechanisms to reduce the amount of information transmitted in an update packet; the simulation assumes that the entire modified record is transmitted to the out-of-date database for synchronization.  While most existing synchronization algorithms do nothing to reduce this data size, the use of data compression and/or delta packets (as described in \ref{update_packet}) can be used to equally reduce the size of packets for any of the synchronization algorithms used (with the notable exception of the null synchronizer, as it lacks any synchronization facilities).  Thus, for the sake of comparison, data reduction methods can be safely ignored within the simulation.

Finally, synchronization always resolves within a single simulation tick, even if it would normally take significantly longer to actually occur.  This should only be a significant issue when synchronizing very large amounts of data on very slow networks, while simultaneously having a significant expected number of synchronizations per unit time.  Synchronizations which occur within seconds of each other should have a significantly small enough probability so as not to affect the final results.  Additionally, these synchronizations can be conceptually moved to a different time within the locale; their results continue to be valid, and would have the same effect on the final result.

None of these limitations are felt to have a significant impact on the final results; the end results continue to permit differing algorithms to be accurately compared to each other.  Removing these implementations would significantly impact the performance of the simulation, without impacting upon the results in a significant or meaningful way.  Thus it is felt that these limitations are sufficiently insignificant enough to have no real impact on the final results.
